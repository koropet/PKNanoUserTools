...
{
	/// <summary>
	/// Автоматическая цепочка размеров по объектам
	/// </summary>
	class DimChain
	{
		//TODO выяснить почему выходит из команды после ключевых слов
		//возможно заработает выбор линии проекции
		//можно добавить проекцию по текущей координатной системе
		
		static double Dim_tolerance = 1d;
		static Line DimDirection;
		bool Auto_direction = true;
		static bool UniteDimms = true;
		static bool tails = true;
		//если крайние размеры на линии, то истинно
		...
		public void Execute()
		{
			...
			dim_line_point = Input.Point("Укажите точку, через которую пройдет размерная линия");
			if (Input.StatusBad)
				return;
		...
		void KeywordInput(object sender, SelectionTextInputEventArgs e)
		{
			...
			if (e.Input == "DIrection") {//направление
				...
				using (TransactionHelper th = new TransactionHelper()) {
					...
					if (!(ent is Line)) {
						App.ShowAlertDialog("Ввели не линию!");
						return;
					}
			...
			if (e.Input == "TOLerance") {
				Dim_tolerance = Input.Double("Введите точность измерения размеров");
				if (Input.StatusBad)
					return;
			}
			if (e.Input == "STeps") {
		...
		void CreateDimms()
		{
			var ds = Platform.ApplicationServices.Application.GetSystemVariable("DIMSCALE");
			double Scale = (double)ds;
			for (int i = 1; i < centers_projected.Count; i++)
			{
				RotatedDimension dim = new RotatedDimension();
				dim.SetDatabaseDefaults(); //для нанокада оказалось важным создать объект и явно назначить ему дефолтные свойства
				//возможно в отличие от автокада что-то добавляется как нулл вызывая при обращении ошибку eNullObjectId
				...
				dim.DimensionStyle = App.DocumentManager.MdiActiveDocument.Database.Dimstyle;
				dim.Annotative = AnnotativeStates.False; //нанокад не бумбум

				App.DocumentManager.MdiActiveDocument.Editor.WriteMessage(dim.XLine1Point.X + " " + dim.XLine2Point.X);
				App.DocumentManager.MdiActiveDocument.Editor.WriteMessage(dim.DimLinePoint.X + " " + dim.DimLinePoint.Y);
				//сужаем тексты
				if (dim.Measurement < Scale * 9 && dim.Measurement > Scale * 4) {
