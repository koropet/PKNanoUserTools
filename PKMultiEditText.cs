...
namespace PKNanoUserTools
{
	class MultiEditText
	{
		...
		public void Execute()
		{
			...
			if(mode==TextEditMode.Copy)
			{
				...
				using( TransactionHelper th = new TransactionHelper())
				{
					...
					{
						App.DocumentManager.MdiActiveDocument.Editor.WriteMessage("Нет текстовых объектов в выборе.");
						return;
					}
					
					foreach(var s in contentCollection)
					{
						App.DocumentManager.MdiActiveDocument.Editor.WriteMessage(s);
					}
				}
				var temp = sset;
				sset = copyTo;
				...
				foreach(var b in blocks_with_attributes)
				{
					...
					foreach(ObjectId attr_o in ac)
					{
						...
						if(!(attribute_names.Contains(attr.Tag)))
						{
							...
							App.DocumentManager.MdiActiveDocument.Editor.WriteMessage("Добавили тэг "+ attr.Tag);
						}
					}
				}
		...
		{
			App.DocumentManager.MdiActiveDocument.Editor.WriteMessage("Ввели ключевое слово " + e.Input);
			...
			if (e.Input == keywords[2])
			{
				...
				if (str_before=="\"\"") str_before=""; //костыль специально для нанокада, потому что автокад понимал кавычки а нанокад долбит их дальше как строку
		...
		string ReadClipboard(string toreplace)
		{
			...
			{
				App.DocumentManager.MdiActiveDocument.Editor.WriteMessage("Пропущен текст не найденный в словаре: "+toreplace);
				return toreplace;
			}
