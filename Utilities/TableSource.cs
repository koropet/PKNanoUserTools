...
namespace PKNanoUserTools.Utilities
{
	...
	public class TableSource
	{
		...
		public TableSource(string FromCSV)
		{
			...
			{
				App.DocumentManager.MdiActiveDocument.Editor.WriteMessage("Проверьте файл. Не совпадает количество ячеек в строках");
				Console.WriteLine(ex);
			}
			catch(Exception ex)
			{
				App.DocumentManager.MdiActiveDocument.Editor.WriteMessage("Неизвестная ошибка");
				Console.WriteLine(ex);
			}
			//Messaging.Tweet("\n" + Make(data));
		}
