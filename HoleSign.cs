...
namespace PKNanoUserTools
{
	...
	class HoleSign
	{
		...
		{
			App.DocumentManager.MdiActiveDocument.Editor.WriteMessage("Делаем два отрезка из двух отрезков");
			...
			else
			{
				...
				App.ShowAlertDialog("Не совпадают концы отрезков");
				status_ok = false;
				return;
			}
			...
			{
				App.ShowAlertDialog("Нулевая длина отрезков");
			...
			if (v3.Length == 0)
			{
				App.ShowAlertDialog("Отрезки параллельны");
				return;
			}

			v3 = v3.MultiplyBy(currscale * Scale/ v3.Length);
		...
		public void Execute()
		{
			...
			{
				input = th.ReadObjects(sset); //нанокад не хочет работать с разными транзакциями
				
				if (input == null || input.Count < 1)
				{
					App.DocumentManager.MdiActiveDocument.Editor.WriteMessage("Не удалось прочитать объекты");
					return;
				}
				...
				for(int i=0;i<lines.Count-1;i+=2)
				{
					...
					App.DocumentManager.MdiActiveDocument.Editor.WriteMessage("Записываем линии в базу данных");
